...
Updates the content for the specified `block_id` based on the block type. Supported fields based on the block object type (see [Block object](https://developers.notion.com/notionapi/reference/block#block-type-object) for available fields and the expected input for each field).
...
>
> To update `child_page` type blocks, use the [Update page](https://developers.notion.com/notionapi/reference/patch-page) endpoint. Updating the page's `title` updates the text displayed in the associated `child_page` block.

> 📘 Updating `child_database` blocks
>
> To update `child_database` type blocks, use the [Update database](https://developers.notion.com/notionapi/reference/update-a-database) endpoint. Updating the page's `title` updates the text displayed in the associated `child_database` block.

> 📘 Updating `children`
>
> A block's children *CANNOT* be directly updated with this endpoint. Instead use [Append block children](https://developers.notion.com/notionapi/reference/patch-block-children) to add children.
...
Returns a 200 HTTP response containing the updated [block object](https://developers.notion.com/notionapi/reference/block) on success.
            ...
            "application/json": {
              "schema": {
                ...
                "properties": {
                  "{type}": {
                    "type": "object",
                    "description": "The [block object `type`](https://developers.notion.com/notionapi/reference/block#block-object-keys) value with the properties to be updated. Currently only `text` (for supported block types) and `checked` (for `to_do` blocks) fields can be updated.",
                    "properties": {}
                  },
                  "archived": {
                    "type": "boolean",
